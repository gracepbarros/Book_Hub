...
const BookCard = ({ book }) => {
  ...
  const fetchShelf = async () => {
    try {
      const response = await axios.get(`http://44.202.24.229.nip.io:3000/shelf?googleID=${book.googleId}&userID=${user.userID}`);
  ...
  const handleShelfUpdate = async (category, favorite) => {
    ...
    try {
      const response = await axios.post("http://44.202.24.229.nip.io:3000/shelf", body);
